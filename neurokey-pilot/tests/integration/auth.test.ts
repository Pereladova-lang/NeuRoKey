...
describe("registration", () => {
  it("creates parent, child and trial subscription", async () => {
    ...
    const res = await register(json({ email, password: "secret123", childName: "Ваня", childAge: 12, childPin: "4321", consent: true }));
  ...
  it("rejects duplicate email with 409", async () => {
    ...
    await register(json({ email, password: "secret123", childName: "А", childAge: 11, childPin: "0000", consent: true }));
    const res = await register(json({ email, password: "secret123", childName: "Б", childAge: 11, childPin: "0000", consent: true }));
    expect(res.status).toBe(409);
  });
  it("rejects registration without consent", async () => {
    const email = `noconsent${Date.now()}@test.ru`;
    const res = await register(json({ email, password: "secret123", childName: "В", childAge: 11, childPin: "0000", consent: false }));
    expect(res.status).toBe(400);
    expect(await db.parent.findUnique({ where: { email } })).toBeNull();
  });
});
...
describe("child login", () => {
  it("sets cookie on correct pin, 401 on wrong", async () => {
    ...
    await register(json({ email, password: "secret123", childName: "Оля", childAge: 13, childPin: "7777", consent: true }));
