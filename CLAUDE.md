...
`DATABASE_URL` and `AUTH_SECRET` must be set (see `.env.example`); DB is Postgres (e.g. via Docker
Compose locally — `postgresql://neurokey:neurokey_dev@localhost:5432/neurokey`).
